...
# Submit Condor jobs for 2DAlphabet SR processing

echo "=========================================="
echo "  Condor Submission for 2DA SR Analysis"
echo "=========================================="
...
if [ ! -f "input_2DA_SR.txt" ]; then
    echo "ERROR: input_2DA_SR.txt not found!"
    exit 1
fi

# Check if template config exists
template_config=$(tail -n +2 input_2DA_SR.txt | head -1 | cut -d',' -f1 | tr -d ' ')
...
njobs=$(tail -n +2 input_2DA_SR.txt | wc -l)
...
condor_submit step7_condor_2DA_SR.cfg
