...
configJSON = "config_Binningv13_Inputv30_SR_BkgMC.json" # sys.argv[2]
...
def plot_fit(signal, tf):
    ...
    twoD.StdPlots('{}-{}_area'.format(signal, tf), subset, lumiText=data_title, pf_slice_str={"fail":"RNNScore < 0.99999","pass":"RNNScore > 0.99999"})
    twoD.StdPlots('{}-{}_area'.format(signal, tf), subset, True, lumiText=data_title, pf_slice_str={"fail":"RNNScore < 0.99999","pass":"RNNScore > 0.99999"})
...
if __name__ == "__main__":
    ...
    for signal, tf_type in zip(signal_areas,tf_types) :
      ...
      useCondor = True
      GOF(signal,tf_type,condor=useCondor)#,extra='--text2workspace --channel-masks --setParameters mask_pass_SIG=1,mask_pass_HIGH=1')
      if useCondor:
