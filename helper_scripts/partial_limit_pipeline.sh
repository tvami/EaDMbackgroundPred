...
#
# Usage:
#   ./partial_limit_pipeline.sh -d LIMITDIR -m MONTHS_OF_LIVETIME [-M MODEL]
#
# Example:
...
MODEL="core"   # detector-volume model passed to limitRateInputScript.py -m

usage() {
    echo "Usage: $0 -d LIMITDIR -m MONTHS_OF_LIVETIME [-M MODEL]"
    echo ""
    echo "  -d   Name for LIMITDIR (e.g. my_test_run)"
    echo "  -m   Number of months of livetime (e.g. 6)"
    echo "  -M   Detector-volume model: core (default) or floating"
    exit 1
}
...
echo ">>> Step 1: python3 helper_scripts/limitRateInputScript.py -d e0 -l ${LIMITDIR} -m ${MODEL}"
...
python3 helper_scripts/limitRateInputScript.py -d e0 -l "${LIMITDIR}" -m "${MODEL}"
echo ""
