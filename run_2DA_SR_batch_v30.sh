...
# The template name is version-tagged, so take it from the argument rather than
# hardcoding it -- a stale literal here silently produces an empty sed output and
# every job then dies on "Expecting value: line 1 column 1".
mv "$1" $rel/src/ || { echo "FATAL: template config '$1' was not transferred"; exit 1; }

# Move histogram directory
echo -e "\n[4c] Moving histogram directory to CMSSW/src"
mv histograms_for_2DAlphabet_v30 $rel/src/
echo "Histogram directory contents:"
ls -lh $rel/src/histograms_for_2DAlphabet_v30/ | head -10
...
if [ ! -d "histograms_for_2DAlphabet_v30" ]; then
    echo "ERROR: histogram directory not found!"
    ls -la
    exit 1
fi
echo "Histogram files:"
ls -lh histograms_for_2DAlphabet_v30/ | grep -E "(${signal}|Data_SR)" || echo "WARNING: Expected files not found"
...
if [ -d "$workingArea" ]; then
    ...
    echo "ERROR: Workspace directory not found -- the fit did NOT produce output."
...
# Exit non-zero on a failed fit. ON_EXIT still transfers the (empty) parent dir, and
# this makes a failure visible in ExitCode instead of 144 green jobs with no output.
exit $rc
